...
void write()
{
    ...
    while (temp != NULL)
    {
        // fprintf(fp, "%s %s %s %lf %d\n", temp->cardname, temp->name, temp->PWD,
        //         temp->money, temp->state);
        fprintf(fp, "%s ", temp->cardname);
        fputs(temp->name, fp);
        fprintf(fp, " %s %lf %d\n", temp->PWD, temp->money, temp->state);
...
void read()
{
    ...
    while (1)
    {
        ...
        flag = fscanf(fp, "%s %s %s %lf %d", temp->cardname, temp->name, temp->PWD, &temp->money, &temp->state);
        if (flag == -1)
        {
            break;
        }
        ...
        if (strcmp(a, temp->cardname) == 0)
        {
            ...
            if (strcmp(b, temp->name) == 0)
            {
                ...
                for (i = 1; i <= 3; i++)
                {
                    ...
                    {
                        printf("您输入的密码正确！\n");
                        printf("\n您的余额是%lf 您的账户状态: %d(1: 账户正常    2: 已销户)\n\n", temp->money, temp->state);
                        break;
                    }
                    else
                    {
...
void transfer()
{
    ...
    while (1)
    {
        if (strcmp(a, temp->cardname) == 0 && temp->state == 0)
        {
            printf("您的卡已销户，请重新开卡！\n");
            break;
        }
